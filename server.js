...
async function api(req, res, url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/stats') return json(res, 200, { donors: store.users.filter(u=>u.role==='donor'&&u.available).length, organizations: store.organizations.length, openRequests: store.requests.filter(r=>r.status==='open'&&new Date(r.neededBy)>new Date()).length, fulfilled: store.requests.filter(r=>r.status==='fulfilled').length });
  ...
  if (req.method === 'GET' && url.pathname === '/api/requests') { const mine=url.searchParams.get('mine')==='true'; const user=mine?auth(req,store):null; if(mine&&!user)return json(res,401,{error:'Please sign in to see your requests.'}); return json(res,200,store.requests.filter(r=>mine?r.ownerId===user.id:r.status==='open'&&new Date(r.neededBy)>new Date()).sort((a,b)=>new Date(a.neededBy)-new Date(b.neededBy))); }
